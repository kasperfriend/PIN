...
public static class NpcVendorService
{
    ...
    /// <summary>Response code a successful purchase carries (the client shows it as the success toast).</summary>
    public const string PurchaseSuccessfulCode = "PURCHASE_SUCCESSFUL";
    ...
    public static VendorProductsResponse BuildProductsResponse(IPlayer player, uint requestedVendorId)
    {
        ...
        Logger.Information(
            "VendorProductRequest: opening vendor {VendorId} ({Title}) with {Products} product(s) for {Player}",
            requestedVendorId,
            title,
            products.Length,
            character);
