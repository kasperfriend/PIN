...
- Fix deployed ground gliders still only playing the wing-open animation and never launching the player. Field logs showed `ForcePush` firing (strength 33) and then `[Glider] Launch handoff ... MoveState=4096 AirTime=32767 Airborne=False VelocityZ=0`: the client stayed standing on the pad, so the 500 ms permission grace expired and the pad retriggered. `ForcedMovement` type 5 is a one-frame velocity impulse (`Time1 = now+19`, `Time2 = now+20`, `ShortTime = CurrentShortTime`) — that is what upstream PIN and the live client send. A previous change stamped the impulse 25 ms in the past *and* set `ShortTime` to that past packet's low 16 bits; the client drops the packet as stale, which is why the animation played and the player did not move. The aptitude launch-pending window that keeps `AirborneDuration` / `RequireMovestate` alive until the first post-impulse pose is unchanged. See `Docs/GLIDER_AND_ADS.md`.
