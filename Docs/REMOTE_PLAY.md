...
| One console says `Server is ready to accept connections on UDP port 25001`, Enter World still hangs, and the log says `Rejected connection … — server not ready (zone still loading)` | The client's game connection reaches a **different** GameServer process than the one that printed ready — a leftover instance from an earlier run still loading (or stuck loading) and owning UDP 25001. Before this build the second instance also failed silently: a process that could not bind the port went on to load the zone and print ready while receiving nothing | `netstat -ano \| findstr :25001` (Windows) or `ss -ulnp` (Linux), stop the PID it names, keep only one GameServer per port. From this build a GameServer that cannot bind its port exits with the reason instead of claiming to be ready |
| GameServer log: `Failed to establish GRPC stream to http://localhost:5201 … (is WebHostManager running?)`, details contain `connection refused` | The GameServerAPI that GameServer dials is hosted by WebHostManager and was not listening when the dial went out — WebHostManager started later, or is not running at all. Logins keep working meanwhile: they fall back to the built-in character data (`Could not get character over GRPC, will use fallback`), and the stream retries every 30 s | Start WebHostManager (or restart it if it crashed); the stream connects on the next retry and character data comes from the account store again |
