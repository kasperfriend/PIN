...
- [Multithreading](MULTITHREADING.md) — what the server runs on worker threads (the navigation bake and the world-population plan build), what deliberately stays on the shard's thread, the `ServerWorkerThreads` / `WorldPopulationPlanOnWorkers` settings, and the determinism contracts that keep a threaded result identical to the single-threaded one
