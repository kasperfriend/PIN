...
- NPC health and AI attack damage come from `dbcharacter::MonsterScaling` at the level of the zone the NPC spawns in (the band top; zones without a band — 12 "Nothing", the untuned mission pockets — resolve at the default player level 1, and a `character_spawn.json` entry can pin an authored `level`); in this build level 45 = health 27,869 with a 13,934 damage rating, level 1 = health 100 and a 50 rating, so every monster spawns with its database stats instead of the old flat 19,192 HP. The damage column is a rating (half the health column on every row), not a per-swing amount: one attack commits `StandardAiRules.AttackDamageFraction` of it (a tenth — 1,393 at level 45, 5 at level 1), and it can only be spent at all inside the mob's melee reach. Player weapon damage is database driven too (item `Damage Per Round` attribute 954 / weapon template `damage_per_round`, grown by the shooter's frame level, with ammo distance falloff), so the hits a level-45 mob survives depend on the weapon and the player's level, not on a 1337 placeholder.
