...
blurry at every setting. The same rule held on the live game, which is why
players of 2014 copied the same three files out of
`dl-production.firefallthegame.com/vtex/<build>/` into their install and turned
streaming off.
...
Three `HEAD` requests, three 404s: **no chunk file answers, so the client keeps
...
ever answered by its literal path: the fallback is scoped to the three chunk names
the client probes for, so it cannot make the host serve something nobody asked
for by a path it invented.

**A set is three files.** `static.vtex0` is the big one, but the client probes
all three and each holds a different level of the page table, so a root with one
or two of them sharpens what they cover and leaves the rest soft. The startup
report says exactly that, naming what is missing:
